...
// solving the puzzle takes (my computer) 0.970s
...
var previousYSentByNat = null

var networkIsIdle = true
...
function runAllComputersTillEnd() {
    ...
    while (true) {
        ...
        if (networkIsIdle) { sendNetPacket() }
    }
}
...
function sendNetPacket() {
    ...
    if (previousYSentByNat == natY) { solution = natY; return }

    previousYSentByNat = natY
    
    const receiver = allComputers[0]

    if (receiver.inputs[0] == -1) { receiver.inputs.shift() }

    receiver.inputs.push(natX)
    receiver.inputs.push(natY)  
...
function runComputer(n) {
    ...
    if (state.inputs[0] != -1) { networkIsIdle = false }
    ...
    while (state.outputs.length > 2) {
        ...
        if (address == 255) { natX = X; natY = Y; continue }
