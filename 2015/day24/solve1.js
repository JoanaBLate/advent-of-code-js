...
// solving the puzzle takes (my computer) 0.820s
...
function main() {
    ...
    if (TARGET != Math.floor(TARGET)) { console.log("Aborting due to input error: total weight is not multiple of 3"); Deno.exit() }
    
    lowestNumberOfPackages = Math.floor(WEIGHTS.length / 3) + 1
...
function fastCheckingForSide(list) {
    ...
    let total = 2 * TARGET // all remaining packages are in list
