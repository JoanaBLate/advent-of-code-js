...
// solving the puzzle takes (my computer) 0.935s
...
function main() {
    ...
    console.log("the answer is", getManhattanDistance())
}
...
function createScanner(id) {
    ...
    return { "id": id, "beacons": [ ], "offsetX": 0, "offsetY": 0, "offsetZ": 0 }
}
...
function alignScanner(scanner, adjustX, adjustY, adjustZ) {
    ...
    scanner.offsetX = adjustX.offset
    scanner.offsetY = adjustY.offset
    scanner.offsetZ = adjustZ.offset
...
function getManhattanDistance() {
    ...
    for (const a of ALIGNEDS) {
        ...
        for (const b of ALIGNEDS) { 
        
            if (a == b) { continue }
            
            const manhattan = Math.abs(b.offsetX - a.offsetX) + Math.abs(b.offsetY - a.offsetY) + Math.abs(b.offsetZ - a.offsetZ)
