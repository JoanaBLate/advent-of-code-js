...
function expandMapHorizontally() {

    for (let row = 0; row < HEIGHT; row++) {

        MAP[row] = expandRowHorizontally(MAP[row])
    }
}

function expandRowHorizontally(source) {

    const list = source.split("").map(function (x) { return parseInt(x) })
...
function expandMapVertically() { 

    const block = [ ]
    
    for (const line of MAP) {
    
        const list = line.split("").map(function (x) { return parseInt(x) })
...
function increaseBlock(block) {

    for (const list of block) { increaseList(list) }
}

function pushBlock(block) {

    for (const line of block) { MAP.push(line.join("")) }
}
