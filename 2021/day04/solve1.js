...
function main() {
    ...
    for (const number of NUMBERS) {
        ...
        winnerBoard = markBoards(number)
        
        if (winnerBoard != null) { winnerNumber = number; break }
    }
...
function markBoards(number) {
    ...
    for (const boardObj of BOARDS) { 
        ...
        for (let n = 0; n < DIM; n++) {
            ...
            if (boardObj.rowPoints[n] == DIM) { return boardObj.board }
            
            if (boardObj.colPoints[n] == DIM) { return boardObj.board }        
        }
    }
    return null
}
