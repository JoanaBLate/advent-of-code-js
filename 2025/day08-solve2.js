// solution for https://adventofcode.com/2025/day/8 part 2
...
function main() {
    ...
    while (true) {
        ...
        createCircuitOrMerge()
        
        if (allCircuits[0].length == 1000) { break }
    }
    
    const indexA = newCircuit[0]
    const indexB = newCircuit[1] 
    
    const boxA = allBoxes[indexA] 
    const boxB = allBoxes[indexB]

    console.log("the answer is", boxA.x * boxB.x)
}
...
function createCircuitOrMerge() {
    ...
    newCircuit = allDistances["" + distance]
...
main()
console.timeEnd("execution time") // 1.2s
