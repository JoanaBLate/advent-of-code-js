...
function main() {
    ...
    while (rawLines.length > 0) { 
        ...
        const rawLine =  rawLines.pop() // backwards
        
        const tokens = rawLine.trim().split(" ")
    
        const instr = tokens.shift()
        
        if (instr == "move")   { moveBackwards(tokens); continue }
        
        if (instr == "reverse") { reverse(tokens); continue }
        
        if (instr == "rotate")  { rotateBackwards(tokens); continue }
        
        if (instr == "swap")    { swap(tokens); continue }        
    }        

    console.log("the un-scrambled version is", data.join(""))
}

function moveBackwards(tokens) {
...
function rotateBackwards(tokens) {

    const kind = tokens.shift() 
    
    if (kind == "left")  { rotateRight(parseInt(tokens.shift())); return }
    if (kind == "right") { rotateLeft(parseInt(tokens.shift())); return }
   
    // rotate based
    const char = tokens.pop()
    
    rotateBasedBackwards(char)
}
...
function rotateBasedBackwards(char) {
