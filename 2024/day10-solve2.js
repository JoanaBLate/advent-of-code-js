// solution for https://adventofcode.com/2024/day/10 part 2
...
function main() {
    ...
    console.log("the answer is", countRatings())
}
...
function continueTrailThis(targetHeight, trailheads, row, col) {
    ...
    for (const trailhead of trailheads) { cell.trailheads.push(trailhead + "|" + row + "~" + col) }    
}
...
function countRatings() {
    ...
    for (let row = 0; row < height; row++) {
        for (let col = 0; col < width; col++) {
            ...
            if (cell.height == 9) { ratings += cell.trailheads.length }
        }
    }
