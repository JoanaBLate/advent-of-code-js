...
function startMinute(state) {
    ...
    if (state.ore >= oreRobotOreCost) { available.push(ORE) }
        
    if (state.ore >= clayRobotOreCost) { available.push(CLAY) }
        
    if (state.ore >= obsidianRobotOreCost  &&  state.clay >= obsidianRobotClayCost) { available.push(OBSIDIAN) }
        
    if (state.ore >= geodeRobotOreCost  &&  state.obsidian >= geodeRobotObsidianCost) { available.push(GEODE) }
...
function search(state, bot) {
    ...
    while (true) {
        ...
        if (! available.includes(bot)) { endMinute(state); continue }
