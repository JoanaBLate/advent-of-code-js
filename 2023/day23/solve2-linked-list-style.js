...
// solving the puzzle takes (my computer) 0.750s
...
function createAndRegisterNode(row, col) {
    ...
    const node = { 
        ...
        "row": row, "col": col, "trips": [ ], "isInPath": false, "tripIndex": -1, "totalDistance": 0,
...
function search() { // depth first search algorithm
    ...
    while (true) { 
        ...
        if (GUARDIAN != null) {
            ...
            if (node != GUARDIAN  &&  GUARDIAN.isInPath) { node = removeNodeFromPath(node); continue } // not going to target
        }
        ...
        if (trip == undefined) { node = removeNodeFromPath(node); continue }
...
function removeNodeFromPath(node) {
    ...
    if (previous != null) { previous.nextNode = null }
...
function addNodeToPath(currentNode, nextNode) {
