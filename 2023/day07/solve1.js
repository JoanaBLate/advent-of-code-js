...
function main() {
    ...
    for (const data of DATA) { 
        ...
        if (kind == "high-card")  { placeData(data, highcards);  continue }    
        if (kind == "one-pair")   { placeData(data, onepairs);   continue }      
        if (kind == "two-pairs")  { placeData(data, twopairs);   continue }            
        if (kind == "threesome")  { placeData(data, threesomes); continue }    
        if (kind == "full-house") { placeData(data, fullhouses); continue }          
...
function getKind(data) {
    ...
    if (labels.length == 5) { return "high-card" }

    if (labels.length == 4) { return "one-pair" }
